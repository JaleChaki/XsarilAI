...
namespace XsarilAI.Handlers {
	public class SubscribeCommandHandler : IHandler {
		...
		public SubscribeCommandHandler(IBehaviorContainer cache, string commandPrefix, string receiveChannelName, string targetChannelName) {
			this.Cache = cache;
		...
		private Task MessageReceiveAsync(SocketMessage message) {
			...
			Cache.Add(new AddPermissionBehavior(guild.Channels.Where(channel => channel.Name == TargetChannelName).FirstOrDefault(), message.Author as SocketGuildUser));
			return Task.CompletedTask;
		}
	}
}
